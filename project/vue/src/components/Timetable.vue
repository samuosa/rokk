...
<template>
  <section
    id="timetable"
    class="border-b-thick border-primary bg-surface-container px-margin-mobile py-12 md:px-margin-desktop md:py-[72px]"
  >
    ...
    <div class="border-brutal hidden bg-white md:block">
      <div class="grid grid-cols-[108px_1fr_1fr] border-b-thick border-primary">
        <div class="border-r-thin border-primary p-3.5 font-mono text-label-caps uppercase text-secondary">TIME</div>
        <div
          v-for="(stage, i) in stages"
          :key="stage.id"
          class="p-3.5 font-display text-[22px] font-extrabold uppercase"
          :class="i === 0 ? 'border-r-thin border-primary' : ''"
        >{{ stage.label }}</div>
      </div>
      ...
      <div
        ...
        :key="time"
        class="grid grid-cols-[108px_1fr_1fr] border-t-thin border-outline-variant"
      >
        <div class="border-r-thin border-primary p-4 font-mono text-[13px] font-bold tracking-[0.08em]">{{ time }}</div>
        <template v-for="(stage, i) in stages" :key="stage.id">
          <div
            class="p-4 transition-colors"
            :class="[
              i === 0 ? 'border-r-thin border-primary' : '',
    ...
    <div class="md:hidden">
      ...
      <div class="flex flex-col gap-3">
        <article v-for="slot in props.slotsFor(activeTab)" :key="slot.id" class="border-brutal bg-white">
          ...
          <div v-if="openSlot === slot.id" :id="`panel-` + slot.id" class="flex flex-col gap-3 border-t-thin border-primary p-4">
