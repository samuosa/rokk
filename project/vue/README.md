...
## SEO
- **Per page** (`useHead` in each view's `setup()`, so it lands in the prerendered HTML rather
  than being injected client-side): `<title>`, meta description, `og:title`/`og:description`/
  `og:type`/`og:url`, `twitter:title`/`twitter:description`, and a canonical link.
- **Site-wide** (`src/App.vue`): favicons, web manifest, `theme-color`, `og:site_name`,
  `og:locale`, the `og:image`/`twitter:image` card and `twitter:card=summary_large_image`.
  These live in `App.vue` rather than `index.html` because each one needs the deploy's base
  path baked in, and `index.html` is a static template. `unhead` dedupes by name/property, so
  a page-level tag always overrides the default.
- **Structured data**: `MusicEvent` on the home page, `MusicGroup` (with the event as
  `performerIn`) on confirmed acts' detail pages. Both build the event node from
  `musicEventJsonLd()` in `src/lib/seo.ts`, so the two can never describe it differently.
  Dates come from `eventMeta.startIso`/`endIso` — never inlined at the call site.
- **`/template`** carries `noindex, nofollow` and is filtered out of the sitemap.
- `scripts/generate-sitemap.mjs` runs as `postbuild` and writes `dist/sitemap.xml` +
  ...
  Note that on a project site the file lands at `/<repo>/robots.txt`, which crawlers do not
  read — only an origin-root `robots.txt` counts, so the `noindex` meta tag is what actually
  keeps `/template` out of the index.

## Icons and the social card
`public/` holds `favicon.svg` (inverts on dark tab bars via `prefers-color-scheme`),
`favicon.ico` (16/32/48), `apple-touch-icon.png`, `icon-192/512(-maskable).png`,
`site.webmanifest` and `og-image.jpg`.

They are generated by `scripts/generate-favicons.py` — a manual tool, **not** part of
`npm run build`; the outputs are committed. Re-run it only when the brand mark changes:

```bash
pip install Pillow && python3 scripts/generate-favicons.py
```

The icon is the circle-A from `src/assets/rokk-logo.jpeg` — the logo's signature glyph, and
the only part of it still legible at 16px (the full wordmark turns to mud). Its proportions are
measured from that artwork. The social card uses the full logo, texture and drips intact, on
the brand's off-white.
